...
lazy_static! {
	static ref RESOLVER: Resolver = crate::ledger_8::test_resolver("simple-merkle-tree");
}
...
impl<D: DB + Clone> Contract<D> for MerkleTreeContract {
	async fn deploy(
		...
		let store_op = crate::ledger_8::contract_operation_new(
			crate::ledger_8::verifier_key(self.resolver, "store").await,
			crate::ledger_8::ir_source(self.resolver, "store").await,
		)?;
		let check_op = crate::ledger_8::contract_operation_new(
			crate::ledger_8::verifier_key(self.resolver, "check").await,
			crate::ledger_8::ir_source(self.resolver, "check").await,
		)?;
